...
import { deleteUnitLabel, updateUnitLabel } from "@/lib/categories";
...
export async function PATCH(
  ...
  try {
    ...
    const category = await updateUnitLabel(organizationId, id, body.name.trim());
...
export async function DELETE(
  ...
  try {
    ...
    requireAdmin(orgRole);
    const { id } = await params;
    await deleteUnitLabel(organizationId, id);
